...
const deletePhysicallyAfter30Days = async () => {
  ...
  try {
    ...
    if (usersToDelete.length > 0) {
      ...
      console.log(
        `Se han eliminado físicamente ${usersToDelete.length} usuarios.`
      );
    } else {
    ...
    if (publicationsToDelete.length > 0) {
      ...
      console.log(
        `Se han eliminado físicamente ${publicationsToDelete.length} publicaciones.`
      );
    } else {
    ...
    if (followsToDelete.length > 0) {
      ...
      console.log(
        `Se han eliminado físicamente ${followsToDelete.length} follows.`
      );
    } else {
